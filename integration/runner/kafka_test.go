...
var _ = Describe("Kafka Runner", func() {
	...
	BeforeEach(func() {
		...
		kafka = &runner.Kafka{
			Name:                  "kafka1",
			ErrorStream:           io.MultiWriter(os.Stdout, errBuffer, GinkgoWriter),
			OutputStream:          io.MultiWriter(os.Stdout, outBuffer, GinkgoWriter),
	...
	It("starts and stops a docker container with the specified image", func() {
		...
		Eventually(outBuffer, 5*time.Second).Should(gbytes.Say(`\Q[KafkaServer id=1] started (kafka.server.KafkaServer)\E`))
